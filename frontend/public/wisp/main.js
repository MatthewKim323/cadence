...
const endCameraPos = { x: 0.1, y: 0.1, z: 0.1 };

// Audio reactivity: 0 = silent/idle, 1 = agent speaking
let audioLevel = 0;
let targetAudioLevel = 0;
const idleTimeSpeed = 0.015;
const speakingTimeSpeed = 0.45;
const idleScale = 0.85;
const speakingScale = 1.1;
...
window.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'AUDIO_LEVEL') {
    targetAudioLevel = Math.min(1, Math.max(0, event.data.level || 0));
  }
});
...
function render() {
  ...
  // Smooth interpolation: ramp up fast (0.08), decay slow (0.03)
  const lerpSpeed = targetAudioLevel > audioLevel ? 0.08 : 0.03;
  audioLevel += (targetAudioLevel - audioLevel) * lerpSpeed;

  if (running || invalidate || isZoomingIn) {
    if (!invalidate) {
      const timeSpeed = idleTimeSpeed + audioLevel * (speakingTimeSpeed - idleTimeSpeed);
      time += dt * timeSpeed;

      const scale = idleScale + audioLevel * (speakingScale - idleScale);
