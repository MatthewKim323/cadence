...
log = logging.getLogger("cadence.zerogpt")
...
async def _run_once(clean_text: str, sentences: list[str]) -> dict:
    ...
    try:
        ...
        log.warning("ZeroGPT timed out")
        return {"score": 0.0, "flagged_indices": []}
    except Exception as e:
        log.error(f"ZeroGPT failed: {e}")
        return {"score": 0.0, "flagged_indices": []}
    finally:
        if session_id:
            try:
...
async def handle_detect(ctx: Context, sender: str, msg: DetectRequest):
    ctx.logger.info(f"ZeroGPT scanning for session {msg.session_id[:16]}...")
    ...
    ctx.logger.info(f"ZeroGPT done: {result['score']:.1f}%, {len(flagged_texts)} flagged")
    await ctx.send(
        sender,
        DetectResult(
...
zerogpt_agent.include(proto, publish_manifest=True)
